...
from openstack.restapi import session as api_session
...
class BaseVersion(object):
    ...
    def client_versions(self, api_versions=[], requested_version=None):
        ...
        if requested_version:
            print "req: %s" % requested_version
            client_versions = [ApiVersion(
                name=self.api_name,
                id=requested_version,
                status=None,
#                 class_name=api_versions[requested_version],
            )]
        else:
            client_versions = []
...
#                     class_name=api_versions[v],
    ...
    def query_server_versions(self, url=None):
        ...
        - strip off the last path component; This handles the old-style auth url
          that ends with a version.
        ...
        version_list = self.query_server(url)

#         if version_list == [] and not strict:
#             # Strip off the entire path
#             version_list = self.query_server("%s://%s" % (u.scheme, u.netloc))
